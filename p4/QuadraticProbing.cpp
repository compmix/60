...
/* 7*/      return currentPos;
        ...
        template <class HashedObj>
        int QuadraticHashTable<HashedObj>::hash( Location key, int tableSize ) const
        {
            if( key.origLocation < 0 ) key.origLocation = -key.origLocation;
            return key.origLocation % tableSize;
        }

        template <class HashedObj>
        int QuadraticHashTable<HashedObj>::hash( StoredBlock key, int tableSize ) const
        {
            if( key.origLocation < 0 ) key.origLocation = -key.origLocation;
            return key.origLocation % tableSize;
        }
